...
zstyle ':vcs_info:*' actionformats '%F{5}%F{2}%b%F{3}|%F{1}%a%c%u%F{5}%f '  # Removed brackets around git status during actions (rebase, merge, etc.)
zstyle ':vcs_info:*' formats '%F{5}%F{2}%b%c%u%F{5}%f '  # Removed brackets around git branch name and status indicators
zstyle ':vcs_info:svn:*' branchformat '%b'
zstyle ':vcs_info:svn:*' actionformats '%F{5}%F{2}%b%F{1}:%F{3}%i%F{3}|%F{1}%a%c%u%F{5}%f '  # Removed brackets for SVN during actions
zstyle ':vcs_info:svn:*' formats '%F{5}%F{2}%b%F{1}:%F{3}%i%c%u%F{5}%f '  zstyle ':vcs_info:*' enable git cvs svn
...
function build_prompt {
    ...
    if in_tmux; then
        prompt_str="$(tmuxbolt) [%F{041}%n%f:%F{blue}%c%f] %{$reset_color%}${vcs_info_msg_0_}%{$reset_color%} ~ "
    else
        prompt_str="$(cancer) [%F{041}%n%f: %~] %{$reset_color%}${vcs_info_msg_0_}%{$reset_color%} ${ARROW_DIRECTION} "
    fi
