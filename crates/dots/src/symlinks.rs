...
/// All symlinks dots manages, sourced from the user's `links.toml` manifest.
///
/// dots ships no built-in personal links: everything is declared by the user
/// via `dots link` (see [`crate::links`]).
pub fn get_symlinks() -> Vec<Symlink> {
    crate::links::planned_symlinks()
}
...
pub fn repair_all() -> Result<RepairReport> {
    Ok(repair_list(&get_symlinks()))
}

/// Check and repair a specific set of symlinks, accumulating a report.
pub fn repair_list(symlinks: &[Symlink]) -> RepairReport {
